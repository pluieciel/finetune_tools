...
def pdftosentence(indoc,outdir):
...
def tsvtolist_3class(tsv,predict_sentence_dir):
    with open(tsv) as tsvfile:
        tsvreader = csv.reader(tsvfile, delimiter="\t")
        list1=list()
        for line in tsvreader:
    ...
    for i in list1:
        ...
        with open(predict_sentence_dir+i[0]+'.txt',encoding='utf-8') as f:
            txt = f.read()
        i[0]=txt
    orderd_list=sorted(list1,key=lambda x:x[1][4])
    #[['txt'
    #   [logit_neg, logit_pos, array_softmax, neg-pos]
    #  ],
    #...]
...
def addhighlight_3class(indoc,outdoc,orderd_list):
    ...
    #pos_list=orderd_list[0:round(len(orderd_list)*pnum-1)]
    #neg_list=orderd_list[::-1][0:round(len(orderd_list)*nnum-1)]
    pos_list=list(filter(lambda x: x[1][3][1]==max(x[1][3]),orderd_list))
    neg_list=list(filter(lambda x: x[1][3][0]==max(x[1][3]),orderd_list))
    for page in doc:
        #pos green
        for text,_ in pos_list:
            text_instances = page.searchFor(text)
    ...
    for page in doc:
        ...
        for text,score in orderd_list:
            ...
            ### scores 
            for inst in text_instances:
                confidence=page.addFreetextAnnot(inst,'p'+str(round(score[3][1]*100,2))+'n'+str(round(score[3][0]*100,2))+'o'+str(round(score[3][2]*100,2)),6,)
                confidence.update(fontsize=6, text_color=[1,0,1], border_color=None, fill_color=None, rotate=-1)
