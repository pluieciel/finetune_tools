    ...
    for label in ["neg", "pos", "other"]:
      ...
      for filename in tf.gfile.ListDirectory(cur_dir):
        ...
        examples.append(InputExample(
            guid=int(re.findall(r'\d+(?=.txt)',filename)[0]), text_a=text, text_b=None, label=label))
...
def get_model_fn(n_class):
  def model_fn(features, labels, mode, params):
    ...
    if mode == tf.estimator.ModeKeys.EVAL:
      ...
      label_ids = tf.reshape(features["label_ids"], [-1])
      guid = tf.reshape(features["guid"], [-1])
      predictions = {
              "guid":guid,
          "logits": logits,
          "labels": label_ids,
...
def main(_):
  ...
  processors = {
      ...
      'sts-b': StsbProcessor,
      'transcript_classification': ImdbProcessor,
...
######################################################################################################################
  if FLAGS.do_predict:
    ...
    with tf.gfile.Open(os.path.join(predict_dir, "{}.tsv".format(task_name)), "w") as fout:
      ...
      est=estimator.predict(input_fn=pred_input_fn,
            yield_single_examples=True,checkpoint_path=FLAGS.predict_ckpt)
      #print("+-+-+",est,pred_input_fn)
      for pred_cnt, result in enumerate(est):
        #print("--+",pred_cnt,result)
        #if pred_cnt % 1000 == 0:
        tf.logging.info("Predicting submission for example: {}".format(pred_cnt),result)
        print("\*******+++Predicting submission for example: {}".format(pred_cnt),result)
        ...
        fout.write("{}\t{}\n".format(result['guid'], result['logits']))
