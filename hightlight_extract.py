# -*- coding: utf-8 -*-
"""
Created on Fri Aug  2 09:50:26 2019
...
class PrettyPrinter:
    ...
    def printall_grouped(self, sections, annots, outfile):
        ...
        highlights_g_raw = [a for a in annots if a.tagname == 'Highlight' and a.gettext() and a.contents is None and a.color[1]>0.9 and a.color[0]<0.9]
        highlights_r_raw = [a for a in annots if a.tagname == 'Highlight' and a.gettext() and a.contents is None and a.color[0]>0.85 and a.color[1]<0.9]
        highlights_y_raw = [a for a in annots if a.tagname == 'Highlight' and a.gettext() and a.contents is None and a.color[0]>0.9 and a.color[1]>0.9]
        global highlight_list,count_gry
        highlight_list=[]
        count_gry=[]
        highlights_g=[]
        highlights_r=[]
        ...
        for s in highlights_g_raw:
            s=self.format_annot(s)
            sublist=re.findall(r'.+?(?<!Mr)(?<!Mrs)(?<!Ms)(?<!\.[^.])(?<![^.]\.)[.!?](?!.\.)(?!com)(?!\d)',s,flags=re.S)
            if len(sublist)==0:
                highlights_g.append(s)
            else:
                highlights_g+=sublist
        for s in highlights_r_raw:
            s=self.format_annot(s)
            sublist=re.findall(r'.+?(?<!Mr)(?<!Mrs)(?<!Ms)(?<!\.[^.])(?<![^.]\.)[.!?](?!.\.)(?!com)(?!\d)',s,flags=re.S)
            if len(sublist)==0:
                highlights_r.append(s)
            else:
                highlights_r+=sublist
        for s in highlights_y_raw:
            s=self.format_annot(s)
            sublist=re.findall(r'.+?(?<!Mr)(?<!Mrs)(?<!Ms)(?<!\.[^.])(?<![^.]\.)[.!?](?!.\.)(?!com)(?!\d)',s,flags=re.S)
            if len(sublist)==0:
                highlights_y.append(s)
            else:
                highlights_y+=sublist
        
        highlight_list=highlights_g+highlights_r+highlights_y
        count_gry=(len(highlights_g),len(highlights_r),len(highlights_y),len(highlight_list))
        ...
        for secname in sections:
            ...
            elif highlights_r and secname == 'highlights_r':
                printheader("Highlights_r")
                for i,tt in enumerate(highlights_r):
                    print(tt)
            ...
            elif highlights_y and secname == 'highlights_y':
                printheader("Highlights_y")
                for i,tt in enumerate(highlights_y):
                    print(tt)
...
def parse_args(inp):
    ...
    allsects = ["highlights_g", "highlights_r", "highlights_y"]
...
outpath='C:/Users/c30557/Desktop/data with labels/'
...
def pdftosentence_exhl(indoc,outdir):
    
    txt=predict_tools.convert_pdf_to_txt(indoc)
    
    txt=re.sub(r'\n',' ',txt)
    txt=re.sub(r'Company Name:.+?\x0c',' ',txt,flags=re.S)
    
    for hltxt in highlight_list:
        try:
            txt=re.sub(hltxt,' ',txt,count=1,flags=re.S)
        except:
            print('***not find***', hltxt)
            info_list.append([indoc,hltxt])
    ...
    for i,s in enumerate(sen):
        #print('*',[s.strip()])
        if i==0:continue
        s=s.strip().encode('utf8')

        f=open(outpath+"other/o_"+args.input.name[args.input.name.rfind('/')+1:]+"_"+str(i)+".txt",'wb')
        f.write(s)
        f.close()
...
for file in f:
    if len(file)-file.rfind('.pdf')<5:
        print('\n',file)
        indoc=path+file
        main(path+file)
        pdftosentence_exhl(indoc,outpath)
        info=[file,count_gry]
