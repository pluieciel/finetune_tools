...
class PrettyPrinter:
    ...
    def printall_grouped(self, sections, annots, outfile):
        ...
        highlights_y = [a for a in annots if a.tagname == 'Highlight' and a.contents is None and a.color[0]>0.9 and a.color[1]>0.9]
        ...
        for secname in sections:
            if highlights_g and secname == 'highlights_g':
                ...
                for i,a in enumerate(highlights_g):
                    tt=self.format_annot(a)
                    print(tt)
                    sublist=re.findall(r'.+?(?<!Mr)(?<!Mrs)(?<!Ms)[.!?](?!\d)',tt,flags=re.S)
                    if len(sublist)==0:
                ...
                for i,a in enumerate(highlights_y):
                    tt=self.format_annot(a)
                    print(tt)
                    sublist=re.findall(r'.+?(?<!Mr)(?<!Mrs)(?<!Ms)[.!?](?!\d)',tt,flags=re.S)
                    if len(sublist)==0:
...
def pdftosentence_exhl(indoc,outdir):
    ...
    pattern = re.compile(r'.+?(?<!Mr)(?<!Mrs)(?<!Ms)[.!?](?!\d)',re.S)
